    ...
    json_t *jn_yuno_settings) // own
{
    ...
    while(gclass_reg) {
        if(!empty_string(gclass_reg->role)) {
            ...
            snprintf(__realm_name__, sizeof(__realm_name__), "%s", realm_name?realm_name:"");
            snprintf(__yuno_role__, sizeof(__yuno_role__), "%s", gclass_reg->role);
...
PUBLIC int gobj_play(hgobj gobj_)
{
    ...
    if(!gobj_is_running(gobj)) {
        // It's auto-starting but display error (magic but warn!).
        log_error(0,
            "gobj",         "%s", gobj_full_name(gobj),
            "function",     "%s", __FUNCTION__,
            "msgset",       "%s", MSGSET_OPERATIONAL_ERROR,
